...
class ViewModelFactory(private val repository: FlowerRepository) : ViewModelProvider.NewInstanceFactory() {
    ...
    override fun <T : ViewModel?> create(modelClass: Class<T>) =  with(modelClass) {
        when {
            isAssignableFrom(FlowerListViewModel::class.java) -> FlowerListViewModel(repository)
            isAssignableFrom(FlowerAddViewModel::class.java) -> FlowerAddViewModel(repository)
    ...
    companion object {
        ...
        fun getInstance(application: Application) =
            INSTANCE ?: synchronized(ViewModelFactory::class.java) {
                INSTANCE ?: ViewModelFactory(FlowerRepository.getInstance(FlowerDatabase.getInstance(application.applicationContext).flowersDao()))
                    .also { INSTANCE = it }
            }
